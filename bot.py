...
async def add_mount(message: types.Message, state: FSMContext):
    await state.update_data(mount_type=message.text)
...
async def add_quantity(message: types.Message, state: FSMContext):
    ...
    await message.answer("Отправьте фото (можно несколько). Когда закончите — /done")
...
async def add_photo(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"Фото {len(photos)} добавлено. /done для завершения.")
...
async def add_done(message: types.Message, state: FSMContext):
    ...
    async with async_session() as session:
        product = Product(
            ...
            quantity=data.get("quantity",0), in_stock=data.get("quantity",0)>0
        )
        session.add(product)
        await session.flush()
        photos = data.get("photos", [])
...
# ... (обработчики для каждого поля и сохранения)
